package com.mengxun.base.config;

import com.mengxun.base.AppiumUtil;
import com.mengxun.base.enums.Environment;
import com.mengxun.base.enums.Platform;
...
//        sleepTime = Integer.valueOf(context.getCurrentXmlTest().getParameter("sleepTime"));
        ...
        if (Platform.ANDROID.getName().equalsIgnoreCase(platformName)) {
            /**
             * 设置和android  测试相关的capability并实例化driver对象
             * */
            if (Environment.TEST.getEnv().equalsIgnoreCase(environment)) {// 测试环境
                //ExcelUtil.excelPath = "TestTnaot_1.xls";
            ...
            } else if (Environment.PRODUCE.getEnv().equalsIgnoreCase(environment)) {// 正式环境
                //ExcelUtil.excelPath = "ProduceTnaot.xls";
...
//            logger.info(PropertiesDataProvider.getTestData(appFilePath, appPackage)+"已经启动");这个地方刚才是这里出的错。这里的意思是获取你配置文件里面的值打印出来。也就是properties里面的。你里面没有写东西所以报错
        ...
        } else if (Platform.ANDROID.getName().equalsIgnoreCase(platformName)) {
            /**
             * 设置和ios  测试相关的capability并实例化driver对象
             * */
            if (Environment.TEST.getEnv().equalsIgnoreCase(environment)) {
                //ExcelUtil.excelPath = "TestTnaot_1.xls";
                File app = new File(classpathRoot, "src/main/resources/Tnaot_test.ipa");
                logger.info("App: " + app.getAbsolutePath());
                capabilities.setCapability("app", app.getAbsolutePath());
            } else if (Environment.PRODUCE.getEnv().equalsIgnoreCase(environment)) {
                //ExcelUtil.excelPath = "ProduceTnaot.xls";
